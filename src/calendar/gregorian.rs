...
impl GregorianDate {
    ...
    pub const fn from_date(date: Date) -> Self {
        ...
        let z = days as i64 + 719_468; // 719468 days from 0000-03-01 to 1970-01-01

        // All contained casts can be shown mathematically not to truncate.
        #[allow(clippy::cast_possible_truncation)]
        #[allow(clippy::cast_sign_loss)]
        {
            let era = (if z >= 0 { z } else { z - 146_096 } / 146_097) as i32;
            let doe = (z - (era as i64) * 146_097) as i32; // [0, 146096]
    ...
    ///
    /// # Errors
    /// Will raise an `InvalidDayOfYear` error if the provided `day_of_year` does not exist within
    /// the provided calendar year.
