...
use crate::Duration;
use crate::{
    FromFineDateTime, HistoricDate, TimePoint, errors::TimePointParsingError, parse::TimeOfDay,
...
where
    ...
    Scale: TimeScale,
{
    type Err = TimePointParsingError<<Self as FromFineDateTime>::Error>;
...
fn check_historic_datetime(
    ...
    let expected_datetime =
        TaiTime::from_datetime(date, hour, minute, second).unwrap() + subseconds;
